/* GLM-5.2 engine (glm_moe_dsa architecture) in plain C.
 ...
 * The DSA indexer is a NO-OP for seq <= index_topk (it selects all keys): dense causal
 * attention is used here -> output identical to the oracle on short prompts.
 *
...
#include "tok.h"
#include "tier.h"
#include "grammar.h"                              /* method F: grammar-constrained drafts (#48) */
...
/* routing precomputed by the GPU (CB layer): moe() uses it and skips PHASE A */
static const int *g_pre_idx; static const float *g_pre_w; static const int *g_pre_keff;
static const float *g_pre_sh;   /* shared expert output already computed on GPU */
#endif
#ifdef __AVX2__
#include <immintrin.h>
...
#include <arm_neon.h>                             /* Apple Silicon / aarch64: NEON kernels */
#elif defined(__VSX__)
#include <altivec.h>                              /* POWER8+ (ppc64le): VSX kernels */
#undef vector                                     /* hygiene: explicit __vector/__bool are used instead */
#undef pixel
#undef bool
#endif
#ifdef __APPLE__
...
/* fmt: 0 F32, 1 INT8, 2 INT4 (2/byte), 3 INT2 (4/byte). q4 holds both packed int4 and int2. */
typedef struct {
...
typedef struct {
    ...
    /* moe (sparse==1) */
    float *router, *router_bias;                 /* router f32 (sensitive) */
    QT sh_gate, sh_up, sh_down;                  /* shared expert */
} Layer;

/* an expert's slot: quantized weights + scales. In the pre-quantized container g/u/d are
 ...
 * experts don't all have the same size (MTP layer int8 = 2x int4 layers). */
...
typedef struct {
    ...
    Layer *L;
    /* COMPRESSED MLA KV-cache: only the normed latent [kv_lora] and k_rot [qk_rope] are
     * kept per token (576 vs 32768 values/token). k_nope and value are reconstructed
     * on the fly with kv_b. This is what makes the context manageable in 15 GB (64 heads, no GQA). */
    ...
    ESlot ws[64];                                /* working set of the current layer (parallel loads) */
    ...
    float **Ic;                                  /* alias of KVState: indexer cache [max_t*hd] */
    int *dsa_sel, *dsa_nsel; int dsa_scap;       /* selection for the current batch's position */
...
static void hwinfo_emit(Model *m);        /* learning cache: defined next to stats_dump */
#ifdef COLI_CUDA
...
#ifdef __APPLE__
    return r.ru_maxrss/(1024.0*1024.0*1024.0);   /* macOS: ru_maxrss in BYTES */
#else
    return r.ru_maxrss/(1024.0*1024.0);          /* Linux: in KB */
#endif
}
static float *falloc(int64_t n){
    /* anti-wrap guard (PR #25 report): an absurd n from a malicious model file must not
     * turn into a small malloc. No calloc: the memset would cost on the hot path. */
...
/* ---- 512-bit int4->float accumulator / 512-bit int4->float accumulator ----
 * Same lossless math as matmul_i4 (nibble->f32, FMA), but 32 weights/iter over
 * two independent FMA chains. NOT bit-identical to the old order: the tree
 * reduction accumulates LESS error than sequential summation (measured 2-4x
 * closer to the double oracle on real shapes; perplexity unchanged, +4-7% on
 * decode with CPU-heavy routing — see docs/experiments/glm52-6x5090-2026-07-12.md).
...
/* selftest against the scalar reference (I4_ACC512_TEST=1): covers nibble
 * ordering and every multiple of 32. / selftest vs the scalar reference. */
...
static int g_i4s=1;   /* SDOT available: int4 IDOT pays off even at S=1 (decode). Measured
                       * on Apple M-series: +14%%, expert-matmul -16%%. EN: with SDOT, int4
...
static int g_i4s=1;   /* POWER8 vec_msum: here the f32 fallback is SCALAR, so int4 IDOT
                       * pays off even at S=1. Measured on a POWER8 S824 (see PR).
...
#endif
/* int8*int8 dot: sign trick (|w| unsigned x*sign(w) signed). Safe:
...
static inline int32_t dot_i8i8(const int8_t *w, const int8_t *x, int I){
    ...
    for(;i+16<=I;i+=16){
        ...
        __m128i xs=_mm_sign_epi8(xv,wv);              /* x * sign(w); _mm_sign requires __AVX2__ */
    ...
    sum=vaddvq_s32(acc);
#elif defined(__VSX__)
    /* POWER8: vec_msum (s8 x u8 -> s32) sums the byte products DIRECTLY into s32
     * lanes, 16 bytes/iter: the 16-bit anti-saturation bound of maddubs isn't needed
     * here. Same sign trick (|w| u8 for x*sign(w) s8), but |w| is built via a MODULO
     * select+sub rather than vec_abs: -128 must wrap to 128 u8, not saturate to 127.
...
/* int4(packed)*int8 dot: nibble -> int8 [-8,7] on the fly, then same trick */
static inline int32_t dot_i4i8(const uint8_t *w4, const int8_t *x, int I){
     ...
     * branch below; 32 elements/iter like there. */
     ...
     * x86 unpacklo/unpackhi (empirically verified on POWER8): the nibbles come out
     * in memory order. |w|<=8 after the -8, so same sign trick as dot_i8i8.
...
static void matmul_qt(float *y, const float *x, QT *w, int S){
     ...
     * pay off (S>=2 gate); but on ARM/SDOT the single token DOES pay off (see g_i4s /
     * PR #9 for the VNNI twin). Threshold configurable via I4S.
...
/* quantize w[O,I] f32 -> packed int4 (2/byte) + scale[O].
...
/* quantize w[O,I] f32 -> packed int2 (4/byte) + scale[O]. 2-bit nibble values in [-2,1]. */
...
static int g_nopack=0;   /* NOPACK=1 -> keeps values <=4bit in an int8 container (to validate packing) */
static int g_drop=0;     /* DROP=1 -> discards expert pages after use. Default 0: leaves them in
                          * the page cache (buff/cache, NOT RSS) as a free L2 -> exploits the
                          * imbalance of MoE routing (a few "hot" experts get reused). */
 ...
 * (#8). NEVER a constraint on sampling: only proposals — the batch-union verification
...
static int g_gr_armed=0;  /* lazy: starts at the first byte allowed from the root (skips preambles) */
...
static int g_looka=0;    /* LOOKA=1: measures (counters only, zero side effects) how predictable
                          * MoE routing is IN ADVANCE — the question that decides whether a
                          * router-driven prefetch can fill the disk's dead time.
                          * [0] previous token, same layer (what SPEC/PREFETCH already use)
                          * [1] layer entry -> routing of the SAME layer (skips attention)
                          * [2] post-attention of layer L -> routing of L+1 (one MoE residual and
                          *     one attention ahead: the point where prefetch would have a full
                          *     disk round-trip to work in the shadow). */
 ...
 *     expert is resident: on a successful load the output stays byte-identical to OFF. */
static pthread_mutex_t g_pilot_mx=PTHREAD_MUTEX_INITIALIZER;
static _Atomic int g_cur_moe_layer=-1;   /* highest moe layer the MAIN has entered (per forward) */
...
/* chooses the format from `bits`: >=16 f32, 5..8 int8, <=4 int4-packed */
...
static void load_cfg(Cfg *c, const char *snap){
     ...
     * first one means generating invisible garbage after the end of the turn (5-10x wasted tokens). */
    c->n_stop=0;
    ...
    /* VALIDATION (PR #25 report): config.json comes from untrusted mirrors — hostile
...
/* builds a [O,I] QT from disk into `t` (buffers reusable across calls).
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
     ...
     * dynamic quants). At bf16 ~1.9GB on real GLM: negligible. dbits>=8 -> f32 here; lower -> dbits. */
    ...
    int NR=c->n_layers+1;                        /* +1: MTP layer row */
    ...
    {
        /* MTP is active ONLY if the set is COMPLETE (tensors live across 3 shards: during a
         * partial conversion only some of them exist). MTP=0 disables it regardless. */
    ...
    /* DSA lightning indexer: active ONLY if the weights (--indexer conversion) are present
     * for ALL full layers. Auto-detection like MTP: no flag, no extra steps. */
    {
...
/* COLI_MMAP=1: experts become VIEWS into mmap of the safetensors files (no pread,
 * no slab, no copy: the kernel's page cache IS the cache). The mappings are
...
/* loads an expert into the slot. Pre-quantized container: the 3 matrices are contiguous in
 * the file -> ONE coalesced ~19 MB pread into `slab` (+ scales in fslab); the QTs are
 * views into the slab (zero copies). Fallback for non-quantized models (tiny oracle).
...
static int expert_load(Model *m, int layer, int eid, ESlot *s, int fatal){
     ...
     * a pread past the mapping means a short-read or silent CORRUPTION of neighbors */
    ...
    if(g_drop){                                  /* drop the pages right away: avoids the page
                                                  * cache under pressure throttling throughput */
...
/* async prefetch of an expert's weights (and its .qs scales): starts the readahead
 * so subsequent synchronous reads find the page cache warm. */
...
/* ---- helper for ABSORPTION: per-row access to quantized QTs ---- */
/* acc[0..I) += coef * W[row,:] (dequant on the fly) */
...
/* MLA attention with compressed KV-cache, on new tokens x[S,hidden], pos_base = position of the first */
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base,
    ...
    /* 1) for each new token: roped query + normed latent and roped k_rot -> into cache.
     ...
     * FULL layer: k_idx of the new tokens cached + top-k selection per query (reused
     * by the following SHARED layers). Selection is active only when context > index_topk
    ...
    if(m->has_dsa && layer<c->n_layers && ((!kvs && m->kv_start[layer]==0) || kvs)){
        ...
        if(c->idx_type[layer]){
            ...
            for(int s=0;s<S;s++){
                ...
                /* top-keep: threshold via descending qsort, then scan in position order */
     ...
     * reconstructed for every context token. By linearity:
    ...
    if(absorb && c->kv_lora<=512){
        ...
        int kvl=c->kv_lora, r0v=c->qk_nope;      /* row offset of V inside the head block */
    ...
    /* 2) reconstruction of k_nope+value for ALL tokens 0..Tk-1 (a single matmul on kv_b) */
     ...
     * (per-thread scores on the heap: see the comment in the absorb branch) */
 ...
 * BATCH-UNION: for S>1 (prefill, MTP verification) each UNIQUE expert of the batch is loaded
 * only once and multiplied for all positions that use it (weights read 1 time);
 * the shared expert is a single S-row matmul. Per-position accumulation stays
 * in order (routed experts in their union order, then shared). */
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    ...
    int sI=c->moe_inter*c->n_shared;
    /* ---- PHASE A: routing of all S positions ---- */
    ...
    for(int base=0;base<nu;base+=64){
         ...
         * this one — the kernel reads in the background, later preads find warm cache */
        if(base+64<nu){
...
#endif
            int nr=0;                                 /* rows (positions) using this expert */
            for(int s=0;s<S;s++) for(int kk=0;kk<keff[s];kk++)
 ...
 * using the SAME pipeline as the real routing (post_ln -> router -> sigmoid+bias, top-K).
...
/* PILOT: router-driven prefetch. Predicts the top-K of layer L+1 from L's post-attention
 * state (recall measured at 71.6% on GLM-5.2, vs 41.3% for the previous token)
 * and launches WILLNEED for the missing experts WHILE L's MoE reads its own: the disk
 * works during compute's dead time instead of waiting for the real routing. With MTP active
 * it predicts for ALL draft positions: speculation drives the I/O too.
 * PILOT_K limits to the top k predictions (the head of the ranking is more reliable
 * than the tail: less bandwidth wasted on wrong predictions).
 *
 * WILLNEED calls are launched from a dedicated I/O THREAD: with the disk queue saturated
 * the fadvise submit BLOCKS (~0.5ms x 169k calls = +92s/48 tokens, measured) — inline
 * the pilot cost more than it delivered. Lock-free 1P/1C ring; full = drop
...
static void pilot_realload(Model *m, int layer, int eid){
    ...
    int slot,isnew;                                     /* grow if there's room, otherwise LRU */
...
/* forward of ONE layer (used by the main layers and the MTP layer) */
static void layer_forward_rows(Model *m, Layer *l, int li, float *x, int S, int pos_base,
     ...
     * in a single GPU submit; the CPU only reads the routing and does resolve/disk/expert-CB.
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base,
    ...
    Cfg *c=&m->c; int D=c->hidden;
    if(g_pilot_real){   /* new forward: layer-ownership restarts at -1 (layers start over from 0) */
        pthread_mutex_lock(&g_pilot_mx);
...
/* METHOD E — prompt-lookup: finds the most recent occurrence of the last bigram in the
 * context and proposes the tokens that followed it. Zero extra weights, zero cost: it's just
 * a hypothesis the model will verify. */
 ...
 * Input: next_tok (just emitted, position kv) and hlast (pre-norm hidden of position kv-1).
...
/* absorbs the VERIFIED pairs (emb(token@pos+1), real_h@pos) into the MTP head's KV:
...
/* ---- METHOD F: grammar draft (#48) ----
 * gr_feed consumes the bytes of every EMITTED token and keeps the walker in sync with the
 * output; grammar_draft proposes the next FORCED span (only one legal byte per position),
 * already tokenized. The tokenization boundary is not guaranteed to match the model's:
 * verification absorbs the difference (worst case the last draft is rejected). */
...
/* clean state at the start of each RESPONSE (not between \x02MORE messages, which continue) */
static void grammar_reset(void){
    if(!g_gr_on) return;
    gr_state_init(&g_gst,&g_gram); g_gr_armed=0;
    if(!g_gst.alive) g_gr_on=0;
}
/* consumes the bytes of an emitted token. Preamble (before arming): ignored.
 * Desync after arming: re-arms waiting for the next valid start — worst case
 * drafts get rejected by verification, the output NEVER changes. */
...
static int grammar_draft(int *draft, int cap){
    ...
    if(g_gr_prop>=32 && g_gr_acc*2<g_gr_prop){        /* adaptive guard, like MTP:
        acceptance below 50% = tokenization off-axis, better to turn off */
        g_gr_on=0;
 ...
 * Leviathan rejection sampling: accept draft x_d with prob p(x_d); on rejection
 * resample from p with x_d zeroed and renormalized. The resulting distribution is
 * EXACTLY p: the speculation stays invisible to the output even with sampling. */
...
/* next token from logits: greedy if g_temp<=0, otherwise sampling.
...
/* active stop-set (populated by run_text/run_serve from config; empty during validation,
 * where a fixed number of tokens is generated to compare against the oracle) */
 ...
 * Each forward verifies up to g_draft tokens proposed from the context: accepted tokens
 * cost only ONE pass over the weights -> disk and RAM bandwidth amortized over more tokens.
 * all: token history (capacity >= kv+n_new+g_draft+2), kv = tokens already in KV.
 * logit = logits of position kv-1 (from prefill); freed here.
 * emit(tok,ud) for each emitted token. Returns the emitted tokens; *kv_out = new kv. */
static int spec_decode(Model *m, int *all, int kv, int n_new, int eos, float *logit,
    ...
    int carry_ban=-1;                    /* token rejected by verification: excluded from the resample */
    while(emitted<n_new && !done){
        ...
        if(!g && g_draft>0){
            /* adaptive auto-off: drafts that are never accepted = pure disk tax */
        ...
        int k=0;                                        /* verification: accept while matching */
        ...
        if(m->has_mtp && k>=1) mtp_absorb(m, all+kv+1, m->h_all, k, kv);   /* MTP KV in sync with verified tokens */
        /* hlast must correspond to the last ACCEPTED position (kv+k), not the end of the batch */
        if(m->h_all && k<S-1) memcpy(m->hlast, m->h_all+(int64_t)k*m->c.hidden, m->c.hidden*sizeof(float));
        kv += 1+k;                                      /* KV beyond kv is stale: it will be overwritten */
...
/* teacher-forcing: a single forward over ids[S], argmax per position into pred[S] */
 ...
 * A single forward per request (teacher-forcing): no generation -> feasible even at low speed. */
...
/* SERVE mode (for the 'coli' CLI): loads the model ONCE, then conversational CHAT.
 * PERSISTENT KV-cache across turns: history stays in cache, only NEW tokens get
 * prefilled -> the model REMEMBERS the conversation and doesn't reprocess the past (lossless,
 * more human, faster). GLM chat template with special tokens (CHAT_TEMPLATE=0 -> raw).
 * Protocol: "\x01\x01" "READY" "\x01\x01\n" after load; streaming response; "\x01\x01" "END" "\x01\x01\n" at end of turn.
 * ":reset" (line "\x02RESET") clears memory. EOF -> exits. */
/* ---- RFC: LIVE RE-PIN (opt-in, REPIN=n, default OFF) ----
 * Upstream does AUTOPIN at START (from .coli_usage history). This adds a re-pin
 * BETWEEN TURNS: at the safe point after the reply, swaps the worst pins for the
 * hottest unpinned ones, so the hot-store tracks the LIVE workload without a separate
 * profile. 25% (+4) hysteresis against ping-pong; max 4 swaps/pass (~20 MB disk each). A
 * separate heat map decays every pass: the persistent .coli_usage history stays intact.
...
/* ---- ON-DISK KV: the conversation reopens WARM (KVSAVE=0 disables it) ----
 ...
 * nrec) and one record per position [tok i32][Lc+Rc of the 78 layers][DSA Ic]. At the
 * end of a turn ONLY the new positions are appended and nrec is rewritten last: a crash
 * mid-append leaves the old nrec = a consistent file. The MTP layer's KV row is not
 * saved: on resume kv_start=-1 and the draft window restarts on its own. */
...
static void run_serve(Model *m, const char *snap){
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(!strcmp(line,"\x02MORE")){                /* continues the response truncated by NGEN:
        ...
        else logit=step(m,hist+len-1,1,len-1);   /* identical/prefix prompt: regenerate the logits */
        EmitStream es={&T,m,now_s(),0,1};
        int prod=0;
        grammar_reset();                         /* new response = new document (MORE continues instead) */
        if(cur>0) prod=spec_decode(m,hist,len,cur,eos,logit,emit_stream,&es,&len);
        else free(logit);
        double tdt=now_s()-tt0; if(tdt<1e-6) tdt=1e-6;
        double dh=(double)(m->hits-h0), dm=(double)(m->miss-ms0);
        ...
        kv_disk_append(m,hist,len);      /* on-disk KV: the next startup resumes from here */
...
/* resident bytes of a [O,I] tensor at the given bit count (mirrors qt_bytes) */
...
/* REAL bytes of an expert: from the container if pre-quantized, otherwise estimated from ebits */
 ...
 * g_ehit[layer][eid]=1 when the expert is routed to in this turn;
 * hits_emit serializes it and clears it. emap_emit snapshots tier+heat of ALL of them. */
...
/* dumps the expert usage histogram to a file: "layer eid count" lines (for PIN).
 * Includes the MTP row (layer n_layers). Atomic write (tmp+rename): also called
 * on every serve turn, and the process can die at any moment. */
 ...
 * Every hit avoids a read from slow disk. */
/* MLOCK: wires pinned experts into physical RAM so macOS's memory compressor
 * can't compress/evict them (observed: real RSS < intended resident -> slow "hits").
 * -1 = auto (ON on macOS where it matters and RLIMIT_MEMLOCK is permissive; OFF
 * elsewhere, where the limit is often tiny and must be raised by hand), 0 = off, 1 = force.
...
#else
    return 0;                                     /* Linux/other: opt-in via MLOCK=1 */
#endif
}
/* Wires [addr,addr+len) into physical RAM. No-op off POSIX (Windows, etc.).
...
/* Wires all pinned-expert slabs (weights + scales) into RAM. Non-fatal on failure.
...
static void pin_load(Model *m, const char *statspath, double gb){
    ...
    pin_wire(m);                                   /* wire into RAM (no compression) */
...
/* RAM available RIGHT NOW (GB): this is the real ceiling, not the total. Linux: MemAvailable
 * from /proc/meminfo. macOS: free+inactive+purgeable pages from host_statistics64
 * (same semantics: reclaimable without swap). Without this branch the fallback
 * "assume 8 GB" crippled the expert cache on exactly the machines with the most RAM. */
...
/* clamps the expert cache to a RAM budget (GB): caps s.t. resident + cache + slack <= budget.
 * ram_gb<=0 -> AUTO budget = 88% of RAM available right now (leaves headroom for OS+wrapper:
...
static void cap_for_ram(Model *m, double ram_gb, int ebits, int max_ctx){
    ...
    if(m->has_mtp) nsp+=2;                       /* MTP cache row: counts ~double (expert int8 = 2x int4) */
    int64_t eb=expert_bytes_probe(m,ebits);
    int auto_b = ram_gb<=0;
    if(auto_b){ ram_gb = g_mem_avail_boot*0.88;   /* measured BEFORE the load: the resident part
                                                   * already allocated is subtracted below, not twice */
     ...
     *  ws[64] working-set slab (ALL materialize during batch-union prefill),
     *  KV cache at max_ctx, kvb_all from attention's k/v reconstruction,
    ...
    /* PAGE-CACHE RESERVE (measured 2026-07-06): starving it collapses buffered
     * preads from ~800 to ~180 MB/s — the last few GB of LRU return LESS than
    ...
    if(capmax < m->ecap){
         ...
         * (cap=8 by default in coli): 23-28% hit rate with tens of GB unused.
         * Capped at n_experts: beyond that, each layer would have slots it can't fill.
...
int main(int argc, char **argv){
    ...
    /* also to stderr: it's the channel that UIs (coli) show to the user */
     ...
     * Must come BEFORE cap_for_ram: pinned experts count towards the resident total. */
     ...
     * at startup the most-used ones get auto-pinned into RAM (half the expert budget: the pin
     * knows YOUR history, the LRU adapts to the session). AUTOPIN=0 disables it. */
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      if(!getenv("PIN") && autopin && hist>=5000){
           ...
           * gets experts wrong and steals slots from the adaptive LRU; at steady state
           * (>=200k selections, a few hours of chat) it reaches half the expert budget. */
      ...
      /* ALWAYS: without a clamp the LRU grows up to cap*76 layers = tens of GB -> OOM-kill.
    ...
    /* otherwise: validation against the oracle (ref_glm.json) */
    ...
    int n_new=nfull-np;
    /* The oracle (ref_glm.json in repo) is for the TINY model: against the 744B it gives
     * a guaranteed 0/20 on EVERY platform (tiny-model prompt tokens = garbage for the real
