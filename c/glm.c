/* GLM-5.2 engine (glm_moe_dsa architecture) in pure C.
 * Stage B: faithful replica of the transformers forward pass (modeling_glm_moe_dsa.py):
 *   - MLA attention (q/kv-LoRA, partial interleaved RoPE)
 *   - sigmoid router + noaux_tc (n_group=1) with routed_scaling_factor
 *   - shared expert + routed experts streamed from disk (per-expert)
 *   - first first_k_dense_replace layers are dense
 * The DSA indexer is a NO-OP for seq <= index_topk (selects all keys): here we use dense
 * causal attention -> output identical to the oracle on short prompts.
 *
 * QUANTIZATION: the experts (streaming) and the resident DENSE part (attention, lm_head,
 * embed, dense mlp, shared expert) are kept in int8 per-row + scale (dequant-on-use).
 * This is what makes GLM-5.2 fit in 15 GB: ~17B resident params at int4 ~= 8.7 GB.
 * Norms/router/bias stay f32 (small and sensitive).
 *
 * Validation: same token ids as ref_glm.json (transformers oracle, c/tools/make_glm_oracle.py).
 *   build: make glm   run: SNAP=./glm_tiny ./glm <cap> <expert_bits> <dense_bits>
 *   TF=1 -> teacher-forcing (validates the prefill over the whole sequence)
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
...
#include <sys/mman.h>                             /* mlock: wire pages into RAM */
#include <sys/stat.h>                             /* fstat for mmap of shards (COLI_MMAP) */
#endif
#include "st.h"
#include "tok.h"
#include "tier.h"
#include "grammar.h"                              /* method F: grammatical drafts (#48) */
...
/* routing precomputed by the GPU (layer CB): moe() uses it and skips PHASE A */
static const int *g_pre_idx; static const float *g_pre_w; static const int *g_pre_keff;
static const float *g_pre_sh;   /* output of the shared expert already computed on GPU */
#endif
#ifdef __AVX2__
#include <immintrin.h>
static inline float hsum256(__m256 v){            /* horizontal sum of 8 floats */
...
#endif
#ifdef __APPLE__
#include <mach/mach.h>                            /* host_statistics64: macOS MemAvailable */
#endif
...
/* tensor [O,I] in one of three formats:
 *   fmt=0 F32   -> qf
 *   fmt=1 INT8  -> q8 (1 byte/param) + per-row scale
 *   fmt=2 INT4  -> q4 (2 values per byte, packed) + per-row scale
 * INT4 is what makes the resident dense part fit in 15 GB (0.5 byte/param). */
/* fmt: 0 F32, 1 INT8, 2 INT4 (2/byte), 3 INT2 (4/byte). q4 holds both int4 and int2 packed. */
typedef struct {
...
/* one expert's slot: quantized weights + scales. In the pre-quantized container g/u/d are
 * VIEWS into `slab` (a single coalesced pread); in the fallback they have their own buffers.
 * slab_cap/fslab_cap: allocated capacity — the ws[] slots are reused ACROSS layers and
 * experts don't all have the same size (MTP int8 layer = 2x the int4 layers). */
...
typedef struct {
    ...
    Layer *L;
    /* COMPRESSED MLA KV-cache: per token we keep only the normed latent [kv_lora] and
     * k_rot [qk_rope] (576 vs 32768 values/token). k_nope and value are reconstructed on
     * the fly with kv_b. This is what makes the 15 GB context manageable (64 heads, no GQA). */
    float **Lc, **Rc; int max_t;                 /* alias of the active KVState */
    int *kv_start;                               /* first valid pos in the layer's KV (MTP: partial) */
    KVState *kv;
    ESlot **ecache; int *ecn; int ecap;          /* per-layer LRU expert */
    ESlot ws[64];                                /* current layer's working set (parallel loads) */
    ...
    uint32_t **elast, eaccess_clock;              /* recency for session-local LFRU */
    /* DSA lightning indexer (active only if the out-idx-* weights are present) */
    int has_dsa;
    QT *ix_wq, *ix_wk, *ix_wp;                   /* per FULL layer: wq_b, wk, weights_proj */
    float **ix_knw, **ix_knb;                    /* k_norm (LayerNorm, eps 1e-6) */
    float **Ic;                                  /* KVState alias: indexer cache [max_t*hd] */
    int *dsa_sel, *dsa_nsel; int dsa_scap;       /* selection for the current batch position */
    /* MTP head (layer n_layers, DeepSeek-V3 style): native drafts with high acceptance */
    int has_mtp; Layer mtpL; QT eh_proj;
    float *enorm, *hnorm, *mtp_norm;
    float *hlast, *h_all;                        /* pre-norm hidden: last pos / all batch positions */
    uint64_t mtp_prop, mtp_acc;                  /* acceptance stats */
    ...
    uint64_t n_fw, n_emit;                       /* method E: decode forwards / tokens emitted */
    double t_edisk, t_ewait, t_emm, t_attn, t_kvb, t_head;/* profiling: where the time goes */
    double t_aproj,t_acore,t_aout;                     /* attention breakdown */
    int64_t resident_bytes;
} Model;

static void usage_save(Model *m);        /* learning cache: defined next to stats_dump */
...
static float *falloc(int64_t n){
    /* anti-wrap guard (report PR #25): an absurd n from a hostile model file must not
     * turn into a small malloc. No calloc: the memset costs on the hot path. */
...
/* ---- 512-bit int4->float accumulator ----
 ...
 * accumulates LESS rounding error than sequential summation (measured 2-4x closer
 * to the double oracle on real shapes; perplexity unchanged, +4-7% on CPU-heavy-
 * routing decode — see docs/experiments/glm52-6x5090-2026-07-12.md).
 * I4_ACC512=0 restores the old order (A/B). */
...
/* selftest against the scalar reference (I4_ACC512_TEST=1): covers nibble order
 * and every multiple of 32. */
...
/* y[S,O] = x[S,I] @ W^T with W quantized int8 per-row + scale[O] (dequant-on-use) */
...
/* y[S,O] = x[S,I] @ W^T with W int4 packed (2 values/byte) + scale[O]. */
    ...
    for (int o=0;o<O;o++){ const uint8_t *w=q4+(int64_t)o*rb; float sc=scale[o];
        for (int s=0;s<S;s++){ const float *xs=x+(int64_t)s*I; float a=0; int i=0;
            ...
            for(;i+16<=I;i+=16){ __m128i by=_mm_loadl_epi64((const __m128i*)(w+(i>>1)));   /* 8 bytes=16 nibbles */
                __m128i lo=_mm_and_si128(by,m4), hi=_mm_and_si128(_mm_srli_epi16(by,4),m4);
                __m128i nib=_mm_unpacklo_epi8(lo,hi);                                       /* nibbles in order */
            ...
            for(;i+16<=I;i+=16){ uint8x8_t by=vld1_u8(w+(i>>1));               /* 8 bytes=16 nibbles */
                uint8x8x2_t z=vzip_u8(vand_u8(by,m4), vshr_n_u8(by,4));        /* nibbles in order */
...
/* y[S,O] = x[S,I] @ W^T with W int2 packed (4 values/byte) + scale[O]. 2-bit nibble -> [-2,1]. */
static void matmul_i2(float *y, const float *x, const uint8_t *q2, const float *scale, int S, int I, int O){
    ...
    for (int o=0;o<O;o++){ const uint8_t *w=q2+(int64_t)o*rb; float sc=scale[o];
        for (int s=0;s<S;s++){ const float *xs=x+(int64_t)s*I; float a=0; int i=0;
            ...
            __m256 acc=_mm256_setzero_ps();
            for(;i+16<=I;i+=16){ __m128i by=_mm_cvtsi32_si128(*(const int*)(w+(i>>2)));    /* 4 bytes=16 values */
                ...
                __m128i nib=_mm_unpacklo_epi16(lo,hi);                                      /* 16 values in order */
            ...
            float32x4_t ac0=vdupq_n_f32(0), ac1=vdupq_n_f32(0);
            for(;i+16<=I;i+=16){ uint32_t wd; memcpy(&wd, w+(i>>2), 4);        /* 4 bytes=16 values */
                ...
                int16x8_t w0=vmovl_s8(vsub_s8(vreinterpret_s8_u16(zz.val[0]),b2v));  /* 16 values in order */
...
/* ---- INTEGER KERNELS (IDOT): activations quantized to int8 per row (absmax/127,
 * Q8_0 style), INTEGER dot product via maddubs/madd AVX2 — no f32 conversion of
 * the weights in the hot loop. ~2-3x on quantized matmuls; added error ~0.3%
 * RMS per matmul (int8 activation), IDOT=0 reverts to the exact f32 path. */
...
static int g_i4s=1;   /* SDOT present: int4 IDOT pays even at S=1 (decode); measured
                       * on Apple M-series: +14%%, expert-matmul -16%%. */
#elif defined(__VSX__)
static int g_i4s=1;   /* POWER8 vec_msum: on VSX the f32 fallback is plain scalar C,
                       * so int4 IDOT pays even at S=1. Measured on a POWER8 S824 (see PR). */
#else
static int g_i4s=2;   /* without SDOT / elsewhere: original threshold (author's AVX2). */
#endif
...
/* hsum of an __m128i with 4 s32 lanes (128-bit AVX-VNNI accumulates in 4 lanes). */
static inline int hsum128_i32(__m128i v){
    v=_mm_hadd_epi32(v,v); v=_mm_hadd_epi32(v,v); return _mm_cvtsi128_si32(v);
}
#endif
/* dot int8*int8: sign trick (|w| unsigned x x*sign(w) signed). Safe:
 * pairs <= 128*127*2 = 32512 < 32767, s32 accumulation up to I=16384. */
static inline int32_t dot_i8i8(const int8_t *w, const int8_t *x, int I){
...
#elif defined(__AVXVNNI__) && defined(__AVX2__)
    /* AVX-VNNI 128-bit: vpdpbusd u8*s8 -> s32, 16 bytes/iter. Same sign trick as
     * the 512-bit variant: |w| via abs, sign folded into x with a mask
     * (w==0 -> product 0). __AVX2__ needed for _mm_sign_epi8 / abs. */
    __m128i acc=_mm_setzero_si128();
    for(;i+16<=I;i+=16){
        __m128i wv=_mm_loadu_si128((const __m128i*)(w+i));
        __m128i xv=_mm_loadu_si128((const __m128i*)(x+i));
        __m128i xs=_mm_sign_epi8(xv,wv);              /* x * sign(w); _mm_sign is __AVX2__ */
    ...
    sum=hsum256_i32(acc);
#elif defined(__ARM_NEON)
    /* ARM: native SDOT if available (Apple Silicon: always); otherwise vmull/vpadal.
     * Same anti-overflow bound as the AVX2 trick: pairs <= 128*127*2 = 32512 < 32767. */
     ...
     * lanes, 16 bytes/iter: maddubs' 16-bit anti-saturation bound isn't needed here.
     * Same sign trick (|w| u8 for x*sign(w) s8), but |w| is built via select+sub MODULO
     * and not vec_abs: -128 must become 128 u8, not saturate to 127.
     * vec_msum accumulates byte products straight into s32 lanes; |w| is built
...
/* dot int4(packed)*int8: nibble -> int8 [-8,7] on the fly, then the same trick */
static inline int32_t dot_i4i8(const uint8_t *w4, const int8_t *x, int I){
     ...
     * (n0/n1), each fed to a 16-byte vpdpbusd. Same 128-bit unpack as the
     * AVX2 branch below; 32 elements/iter like there. */
    ...
    for(;i+32<=I;i+=32){
        ...
        __m128i lo=_mm_and_si128(by,m4), hi=_mm_and_si128(_mm_srli_epi16(by,4),m4);
        __m128i n0=_mm_unpacklo_epi8(lo,hi), n1=_mm_unpackhi_epi8(lo,hi);  /* nibbles in order */
    ...
    sum=vaddvq_s32(acc);
#elif defined(__VSX__)
    /* 16 bytes = 32 nibbles. vec_mergeh/l on ppc64le interleave like x86 unpacklo/hi
     * (verified empirically on POWER8): nibbles come out in memory order. |w|<=8 after
     * the -8, so same sign trick as dot_i8i8. */
...
/* allow_idot=0 forces the EXACT int4/int8 kernel (f32 activations). The attention
...
static void matmul_qt_ex(float *y, const float *x, QT *w, int S, int allow_idot){
    ...
    /* int8 IDOT always wins (1.4-2.5x). int4 IDOT: on AVX2 the author found S=1 didn't
...
/* quantize w[O,I] f32 -> int8 q[O,I] + symmetric per-row scale[O] */
...
/* quantize w[O,I] f32 -> int4 packed (2/byte) + scale[O].
...
/* quantize w[O,I] f32 -> int2 packed (4/byte) + scale[O]. 2-bit nibble values in [-2,1]. */
...
static int g_nopack=0;   /* NOPACK=1 -> keeps <=4bit values in an int8 container (to validate packing) */
static int g_drop=0;     /* DROP=1 -> discards expert pages after use. Default 0: leaves them in
                          * page cache (buff/cache, NOT RSS) as a free L2 -> exploits MoE routing's
                          * imbalance (a few "hot" experts get reused). */
static int g_prefetch=0; /* PREFETCH=1 -> re-enables cross-layer WILLNEED (method C). Default
                          * OFF: the real parallel loads made it superfluous, and under memory
                          * pressure the speculative readahead was getting re-evicted anyway. */
static int g_direct=0;   /* DIRECT=1 -> O_DIRECT on the expert slabs. Default OFF: on this host
                          * (VHDX on DRAM-less NVMe, ~60ms/req serialized latency) plain buffered
                          * I/O turned out best; on real NVMe DIRECT=1 pays off more. */
static float g_temp=-1;  /* TEMP: token sampling temperature. <0 = auto (1.0 in chat/text,
                          * 0=greedy in validation). 0 = pure greedy. */
static float g_nuc=0.95f;/* NUCLEUS: top-p over the vocabulary (default from GLM-5.2's generation_config) */
static int g_topk=0;     /* TOPK=n -> use n experts/token instead of config (research: less disk) */
static float g_topp=0;   /* TOPP=p (0..1) -> adaptive top-p: keep experts up to cumulative weight p */
static int g_spec=1;     /* method C: SPEC=0 disables cross-layer speculative prefetch */
static int g_draft=0;    /* method E: DRAFT=n tokens auto-speculated per forward via n-gram lookup
                          * (0=off). LOSSLESS: verification = output identical to greedy. Default OFF:
                          * measured on a real run (2026-07-03) acceptance ~5% -> every rejected
                          * draft still pays for its experts from disk = ~3x slower.
                          * Opt-in (DRAFT=4) for repetitive text where acceptance is high. */
/* method F (#48): GRAMMAR=<file.gbnf> -> a third draft source, the grammar itself.
 * In constrained-output workloads (JSON/NDJSON, function calling) the bytes FORCED by
 * the grammar (keys, punctuation, enum values) are free drafts with acceptance ~1:
 * no head, no lookup table, and it kicks in even where the int4 MTP head doesn't
 * fire (#8). NEVER a constraint on sampling: only proposals, the batch-union
 * verification decides — wrong grammar = rejected drafts, identical output.
 * GRAMMAR_DRAFT=n (default 24) caps the forced tokens per forward. */
static Grammar g_gram; static GrState g_gst;
static Tok *g_gr_T=NULL;
static int g_gr_on=0;     /* grammar loaded and walker alive */
static int g_gr_armed=0;  /* lazy: starts from the first byte the root admits (skips preambles) */
...
static int g_looka=0;    /* LOOKA=1: measures (counters only, zero effect) how far AHEAD the MoE
                          * routing is predictable — the question that decides whether a
                          * router-driven prefetch can fill the disk's dead time.
                          * [0] previous token, same layer (what SPEC/PREFETCH already use)
                          * [1] layer input -> routing of the SAME layer (skips attention)
                          * [2] post-attention of layer L -> routing of L+1 (one MoE residual and
                          *     one attention ahead: the point where a prefetch would have a
                          *     whole disk round-trip to work in the shadows). */
...
static int g_pilot_k=8;  /* PILOT_K=k: only prefetch the top k predictions per position */
...
static int g_pilot_real=0;/* PILOT_REAL=1: the pilot does REAL cross-layer loads into ecache[L+1]
                          * (not just WILLNEED). Implies PILOT=1. Default OFF: hint-only. */
/* main<->pilot handshake for the real cross-layer load. Safety invariant in TWO parts:
 *  1) MATMUL path (moe): the pilot writes ONLY ecache[layer] with layer > g_cur_moe_layer;
 *     the matmul in moe() reads ONLY ecache[layer]==g_cur_moe_layer, and the barrier at the
 *     start of moe() waits for any in-flight load on THAT layer. So NO half-loaded slot is
 *     ever matmul'd: the matmul and the pilot never touch the same layer at the same time.
 *  2) SCAN path (pilot_prefetch, also on MAIN): the residency scan runs over the FUTURE
 *     layer (lnext = current layer + 1), exactly the layer the pilot is writing to
 *     -> HERE the two threads genuinely touch the same ecache. So that scan takes
 *     g_pilot_mx (the same lock as the worker): slot reads and publication are serialized,
 *     no torn reads of ecn[]/eid. The pilot NEVER alters an expert's value, only WHICH
 *     expert is resident: a successful load leaves the output byte-identical to OFF. */
static pthread_mutex_t g_pilot_mx=PTHREAD_MUTEX_INITIALIZER;
static _Atomic int g_cur_moe_layer=-1;   /* highest moe layer MAIN has entered (per forward) */
static _Atomic int g_pilot_inflight=-1;  /* layer the worker is REAL-loading right now (-1 = idle) */
...
/* picks the format from `bits`: >=16 f32, 5..8 int8, <=4 int4-packed */
...
/* interleaved RoPE on a vector of size qk_rope at position pos */
...
static void load_cfg(Cfg *c, const char *snap){
    ...
    /* stop tokens: GLM-5.2 has THREE (endoftext, user, observation). Stopping only on the
     * first means generating invisible garbage after the end of the turn (5-10x wasted tokens). */
    c->n_stop=0;
    ...
    /* VALIDATION (report PR #25): config.json comes from untrusted mirrors — hostile
     * dimensions must not get past this point. A single choke point protects every downstream alloc. */
...
/* builds a QT [O,I] from disk into `t` (buffers reusable across calls).
 *  - if `name.qs` exists: weights ALREADY quantized in the container (U8 qdata + F32 scale) -> read directly
 *  - otherwise: full tensor (f32/bf16) -> quantized at runtime to `bits` (tiny oracle / full weights)
...
static void qt_from_disk(Model *m, const char *name, int O, int I, int bits, int drop, QT *t){
    ...
    if(st_has(&m->S,sn)){
        ...
        int fmt = (nb==(int64_t)O*I)?1 : (nb==(int64_t)O*((I+1)/2))?2 : 3;  /* int8 / int4 / int2 from byte count */
...
static float *ld(Model *m, const char *name){   /* resident 1D f32 tensor (norms/bias) */
...
static void model_init(Model *m, const char *snap, int cap, int ebits, int dbits){
    ...
    /* embed and lm_head are the I/O boundary: keep them at high precision (like real
     * dynamic quants do). At bf16 ~1.9GB on real GLM: negligible. dbits>=8 -> f32 here; lower -> dbits. */
    ...
    for(int i=0;i<c->n_layers;i++){
        ...
        if(!l->sparse){
            ...
            m->eroute[i]=calloc(c->topk,sizeof(int));      /* method C: layer's last routing */
    ...
    /* MTP head (layer n_layers): present only if converted with --mtp */
    {
        /* MTP is active ONLY if the set is COMPLETE (the tensors live across 3 shards:
         * a partial conversion only has part of them). MTP=0 disables it regardless. */
        ...
        if(m->has_mtp){
            ...
            m->eheat[i]=calloc(c->n_experts,sizeof(uint32_t));
            m->elast[i]=calloc(c->n_experts,sizeof(uint32_t));
            m->kv_start[i]=-1;                    /* MTP KV: starts at the first decode position */
            #undef PM
        }
    }
    /* DSA lightning indexer: active ONLY if the weights (--indexer conversion) exist for
     * ALL full layers. Auto-detected like MTP: no flag, no extra steps. */
    {
    ...
    /* bytes of the resident DENSE part (embed+lm_head+attn+dense mlp+shared+norms) */
...
/* embed: dequantize the token's row (per-row scale) into x[hidden] */
...
/* COLI_MMAP=1: experts become VIEWS into the mmap of the safetensors files (no pread,
 * no slab, no copy: the kernel's page cache IS the cache). The maps are
 * registered with Metal (newBufferWithBytesNoCopy on file-backed pages, like llama.cpp),
 * so the GPU reads the same bytes. Falls back to the slab path on misalignment. */
static int g_mmap=0;
static struct { int fd; void *base; size_t len; } g_maps[512]; static int g_nmaps;
static pthread_mutex_t g_map_mtx = PTHREAD_MUTEX_INITIALIZER;   /* expert_load is OMP-parallel */
...
/* loads one expert into the slot. Pre-quantized container: the 3 matrices are contiguous
 * in the file -> ONE coalesced ~19 MB pread into `slab` (+ scales in fslab); the QTs are
 * views into the slab (zero copies). Fallback for unquantized models (tiny oracle).
 * THREAD-SAFE across distinct slots (positional pread, read-only st_find). */
...
static int expert_load(Model *m, int layer, int eid, ESlot *s, int fatal){
    ...
    if(!st_has(&m->S,qn)){                       /* fallback: full tensors, quantize at runtime.
     ...
     * pread past the mapping = short-read or silent CORRUPTION of neighbors */
    ...
    int ord[3]={0,1,2};                          /* sort by offset in the file */
    ...
    if(contig){
        ...
        if(dfd>=0){                              /* O_DIRECT: offset/len aligned to 4K */
    ...
    if(!done){                                   /* non-contiguous: 3 buffered preads */
        int64_t o=0;
    ...
    float *fp[3]; int64_t fo=0;                  /* scales (small) */
    for(int k=0;k<3;k++){
    ...
    if(g_drop){                                  /* drop the pages immediately: avoids the page
                                                  * cache under pressure strangling throughput */
...
/* async prefetch of an expert's weights (and its .qs scales): kicks off readahead
 * so the following synchronous reads find a warm page cache. */
...
/* y[0..n) = W[r0+j,:]*x  (matvec over a SLICE of the QT's rows) */
...
static int g_absorb=-1;   /* ABSORB: -1 auto (decode S<=4), 0 never, 1 always (test) */
...
/* MLA attention with compressed KV-cache, on new tokens x[S,hidden], pos_base = pos of the first */
...
static void attention_rows(Model *m, Layer *l, int layer, float *x, int S, int pos_base,
    ...
    /* 1) roped query + normed latent and roped k_rot -> into cache.
     * QR holds the q_a residual for ALL positions: also needed by the DSA indexer.
     *
     * BATCH-ROWS: batch the three projections over all S rows, like o_proj below and moe()'s
    ...
    for(int s=0;s<S;s++){ float *qr=QR+(int64_t)s*c->q_lora;
        rmsnorm(qr, qr, l->q_a_ln, c->q_lora, c->eps); }         /* q_b reads the NORMED residual */
    ...
    for(int s=0;s<S;s++){
        ...
        rmsnorm(Ldst, Ldst, l->kv_a_ln, c->kv_lora, c->eps);     /* normed latent */
        memcpy(Rdst, cs+c->kv_lora, c->qk_rope*sizeof(float));
        rope_interleave(Rdst, pos, c);                            /* roped k_rot, shared across heads */
    }
    /* ---- DSA lightning indexer ----
     * FULL layer: k_idx of the new tokens in cache + top-k selection per query (reused
     * by the subsequent SHARED layers). Selection active only with context > index_topk
     * (or DSA_FORCE=1 for testing: selecting EVERYTHING must give the exact dense output). */
    ...
    if(m->has_dsa && layer<c->n_layers && ((!kvs && m->kv_start[layer]==0) || kvs)){
        ...
        if(c->idx_type[layer]){
            /* BATCH-ROWS, like the attention projections above: batch ix_wk over all S rows
             * instead of re-reading it per token; allow_idot=0 keeps it on the exact int4
             * kernel so the result stays bit-identical. */
            ...
            for(int s=0;s<S;s++){
                ...
                /* top-keep: threshold via qsort desc, then scan in position order */
    ...
    /* WEIGHT ABSORPTION (DeepSeek): for small S (decode/MTP verification) k/v is NOT
     * reconstructed for every token in the context. By linearity:
     *   q·k_nope_t = (W_K^hT q_nope)·L_t      ctx^h = W_V^h (Σ_t a_t L_t)
     * cost per step ~O(T·kv_lora) instead of O(T·H·(nope+vh)) for the kvb_all matmul. */
    int absorb = kvs || g_absorb==1 || (g_absorb<0 && S<=4);
    if(absorb && c->kv_lora<=512){
        m->t_aproj+=now_s()-ta0; double tac=now_s();
        int kvl=c->kv_lora, r0v=c->qk_nope;      /* V row offset within the head block */
        /* Per-thread scores on the HEAP. The cap MUST be the batch's actual max nt,
         * not Tk+1: Tk=pos_base+S only holds when pos==pos_base+s. The batched path
         * (step_decode_batch from run_serve_mux) passes per-slot positions[] and
         * kv_start, so nt=pos+1-st0 can exceed Tk+1 -> heap-buffer-overflow on
         * sc[jj]. Counted exactly like the loop below. */
        int64_t sc_cap = 1;
        for(int s=0;s<S;s++){
            KVState *ks=kvs?kvs[s]:m->kv;
            int pos=positions?positions[s]:pos_base+s;
    ...
    /* 2) reconstruct k_nope+value for ALL tokens 0..Tk-1 (a single matmul on kv_b) */
     ...
     * (scores on the heap, per-thread: see the comment in the absorb branch) */
 ...
 * BATCH-UNION: for S>1 (prefill, MTP verification) each UNIQUE expert in the batch is
 * loaded once and multiplied against all the positions that use it (weights read once);
 * the shared expert is a single S-row matmul. Per-position accumulation stays
 * in order (routed in their union order, then shared). */
static void moe(Model *m, Layer *l, int layer, float *x, int S, float *out){
    if(g_pilot_real){   /* cross-layer barrier: take possession of THIS layer and wait for
                         * any in-flight pilot load on the same layer (after which the
                         * worker drops any new load <= layer -> ecache[layer] is stable
                         * for the whole resolve/matmul/promotion below). */
        for(;;){
    ...
    if(g_looka && S==1 && layer<c->n_layers){
        ...
        for(int kind=0;kind<2;kind++) if(la_val[kind][layer]){   /* [1]/[2] vs predictions */
    ...
    /* ---- PHASE C/D: resolve (pin/cache/disk) and compute, in blocks of 64 uniques ---- */
    ...
    for(int base=0;base<nu;base+=64){
        ...
        /* ASYNC I/O: readahead (WILLNEED) of the NEXT block while we compute
         * this one — the kernel reads in the background, later preads find a warm cache */
        if(base+64<nu){
        ...
        { ESlot *Sl=m->ecache[layer]; int *nn=&m->ecn[layer];   /* LRU promotion (swap buffer) */
    ...
    /* ---- PHASE E: shared expert, a single S-row matmul (skipped if fused into the GPU block) ---- */
...
/* LOOKA: predicts the top-K of layer `target`'s router from state h (residual stream),
 * using the SAME pipeline as real routing (post_ln -> router -> sigmoid+bias, top-K).
 * kind 0 = same layer skipping attention, kind 1 = next layer. */
...
/* PILOT: router-guided prefetch. Predicts layer L+1's top-K from L's post-attention
 * state (recall measured at 71.6% on GLM-5.2, vs 41.3% for the previous token)
 * and fires WILLNEED on the missing experts WHILE L's MoE reads its own: the disk
 * works during compute's dead time instead of waiting for real routing. With MTP
 * active it predicts for ALL draft positions: speculation drives the I/O too.
 * PILOT_K limits it to the first k predictions (the head of the ranking is more
 * reliable than the tail: less bandwidth wasted on wrong predictions).
 *
 * WILLNEEDs are fired from a dedicated I/O THREAD: with the disk queue saturated the
 * fadvise submit BLOCKS (~0.5ms x 169k calls = +92s/48 tokens, measured) — inline
 * the pilot cost more than it paid off. Lock-free 1P/1C ring; full = drop
 ...
 * the lock only protects slot selection/publication and the handshake with main. */
static void pilot_realload(Model *m, int layer, int eid){
    ...
    int slot,isnew;                                     /* grow if there's room, else LRU */
    ...
    int rc=expert_load(m,layer,eid,dst,0);              /* REAL pread — outside the lock, overlapped with compute; fatal=0: an error on a speculation must NOT kill the server */
...
/* score + enqueue: called from moe() after PHASE A with the position's routed set */
static void couple_prefetch(Model *m, int layer, const int *idx, int Ke){
...
/* forward of ONE layer (used by the main 78 and by the MTP layer) */
...
#ifdef COLI_METAL
    /* FULL-LAYER CB: in_ln + attention + residual + post_ln + shared expert + router/top-K
     * in a single GPU submit; the CPU reads the routing and only does resolve/disk/expert-CB.
     * Fallback: any missing condition -> the whole CPU path below. */
    if(g_metal_enabled && S<=4 && li<c->n_layers && l->sparse
       ...
       && c->n_experts==256 && c->topk==8 && c->n_shared==1 && c->moe_inter==2048){
        ...
        if(!sel_active){
            ...
            if(ok){
                m->t_attn += now_s()-ta0;
                if(m->has_dsa && c->idx_type[li]){            /* index key for future selections */
...
static void layers_forward_rows(Model *m, float *x, int S, int pos_base,
    ...
    Cfg *c=&m->c; int D=c->hidden;
    if(g_pilot_real){   /* new forward: layer-ownership resets to -1 (layers start over from 0) */
        pthread_mutex_lock(&g_pilot_mx);
    ...
    for(int i=0;i<c->n_layers;i++){
        /* progress on stderr for large batches (prefill): the first response byte
         * can arrive after MINUTES of streaming — in the dark it looks like a hang. */
...
static void kv_alloc(Model *m, int max_t){
    ...
    k->max_t=max_t;
    int NR=c->n_layers+1;                        /* extra row: MTP layer's KV */
...
/* like step(), but returns the logits of ALL S positions [S,vocab] (for spec verification) */
static float *step_all(Model *m, const int *ids, int S, int pos_base){
    Cfg *c=&m->c; int D=c->hidden;
    float *x=falloc((int64_t)S*D);
    for(int s=0;s<S;s++) embed_row(m, ids[s], x+(int64_t)s*D);
    layers_forward(m,x,S,pos_base);
    if(m->h_all) memcpy(m->h_all, x, (int64_t)S*D*sizeof(float));   /* hidden of ALL positions (S<=64) */
    if(m->hlast) memcpy(m->hlast, x+(int64_t)(S-1)*D, D*sizeof(float));
...
/* METHOD E — prompt-lookup: searches for the most recent occurrence of the last
 * bigram in the context and proposes the tokens that followed it. Zero extra
 * weights, zero cost: it's just a hypothesis the model will verify. */
...
/* METHOD MTP: proposes up to G drafts with GLM-5.2's native multi-token head.
 * Input: next_tok (just emitted, position kv) and hlast (pre-norm hidden of pos kv-1).
 * DeepSeek-V3 chain: h' = Layer78( eh_proj[ enorm(emb(tok)) ; hnorm(h) ] ),
 * draft = argmax(lm_head(shared_head.norm(h'))). The MTP layer's KV lives at row n_layers
 * and is valid from kv_start (no prefill: decode-only window, enough for the draft). */
...
static int mtp_draft(Model *m, int next_tok, int kv, int G, int *draft){
    ...
    for(int g=0; g<G; g++){
        ...
        if(g==0 && !prenorm) rmsnorm(h, h, m->final_norm, D, c->eps);  /* true h: post model.norm */
...
/* absorbs the VERIFIED pairs (emb(token@pos+1), true_h@pos) into the MTP head's KV:
 * next_ids[i] = token at position pos_base+i+1; x[i] = TRUE hidden at pos_base+i.
 * A single batched pass of the MTP layer (batch-union makes the experts cheap). */
static void mtp_absorb(Model *m, const int *next_ids, const float *x, int S, int pos_base){
...
/* ---- METHOD F: grammatical draft (#48) ----
 * gr_feed consumes the bytes of every EMITTED token and keeps the walker in sync
 * with the output; grammar_draft proposes the next FORCED span (one legal byte
 * per position), already tokenized. The tokenization boundary is not guaranteed
 * to match the model's: verification absorbs the difference (worst case the
 * last draft is rejected). */
...
/* clean state at the start of every RESPONSE (not between \x02MORE, which continue) */
static void grammar_reset(void){
    if(!g_gr_on) return;
    gr_state_init(&g_gst,&g_gram); g_gr_armed=0;
    if(!g_gst.alive) g_gr_on=0;
}
/* consumes the bytes of one emitted token. Preamble (before arming): ignored.
 * Desync after arming: re-arms waiting for the next valid start — worst case
 * the drafts get rejected by verification, the output NEVER changes. */
static void gr_feed(int t){
    ...
    for(int i=0;i<n;i++){
        ...
        if(r<0){ g_gr_on=0; return; }                 /* walker dead: end of drafts */
...
/* proposes the forced span as tokens (max cap); 0 if the grammar branches here */
static int grammar_draft(int *draft, int cap){
    if(!g_gr_on||!g_gr_armed||!g_gr_T||cap<1) return 0;
    if(g_gr_prop>=32 && g_gr_acc*2<g_gr_prop){        /* adaptive guard, like for MTP:
        acceptance below 50% = tokenization off-axis, better to disable */
        g_gr_on=0;
 ...
 * Leviathan rejection sampling: accepts draft x_d with prob p(x_d); on rejection
 * resamples from p with x_d zeroed and renormalized. The resulting distribution is
 * EXACTLY p: speculation stays invisible to the output even with sampling. */
...
/* builds the target distribution in g_pbuf: softmax(lo/temp) truncated to top-p g_nuc */
...
/* samples from g_pbuf; ban>=0 -> that token is excluded (renormalizing on the fly) */
 ...
 * ban = token excluded because it was rejected by the previous speculative verification. */
...
/* active stop-set (populated by run_text/run_serve from config; empty during
 * validation, where a fixed number of tokens is generated to compare against the oracle) */
 ...
 * Each forward verifies up to g_draft tokens proposed from the context: accepted
 * tokens cost a SINGLE pass over the weights -> disk and RAM bandwidth amortized
 * over multiple tokens.
 * all: token history (capacity >= kv+n_new+g_draft+2), kv = tokens already in KV.
 * logit = logits of position kv-1 (from the prefill); freed here.
 * emit(tok,ud) for every emitted token. Returns the emitted tokens; *kv_out = new kv. */
static int spec_decode(Model *m, int *all, int kv, int n_new, int eos, float *logit,
    ...
    int carry_ban=-1;                    /* token rejected by verification: excluded from resample */
    while(emitted<n_new && !done){
        ...
        gr_feed(next);                                  /* the walker follows the emitted output */
        if(emitted>=n_new) break;                       /* no need to forward the last token */
        ...
        if(!g && g_draft>0){
            /* adaptive auto-off: drafts that are never accepted just tax the disk */
        ...
        if(m->has_mtp && k>=1) mtp_absorb(m, all+kv+1, m->h_all, k, kv);   /* MTP KV in sync with verified */
        ...
        kv += 1+k;                                      /* KV beyond kv is stale: will be overwritten */
...
/* emit callback: accumulates into an array (validation) */
typedef struct { int *dst; int n; } EmitStore;
static void emit_store(int t, void *ud){ EmitStore *e=(EmitStore*)ud; e->dst[e->n++]=t; }
/* emit callback: detokenizes and prints in streaming (chat/run), with heartbeat */
...
/* teacher-forcing: a single forward on ids[S], argmax per position into pred[S] */
...
/* log-prob (log-softmax) of the target token given the logit vector; *am=1 if it's the argmax */
...
static void profile_print(Model *m, double elapsed){
    ...
    if(g_metal_enabled){ uint64_t ok=0,fb=0,ex=0; double su=0,gp=0,sc=0;
        ...
        printf("METAL: GPU blocks %llu | CPU fallback %llu | experts on GPU %llu | setup %.2fs gpu-wall %.2fs (kernel %.2fs) scatter %.2fs\n",
...
/* real generation: tokenizes PROMPT, prefill + greedy decode with stop on EOS,
 * detokenizes and prints the text in streaming. */
...
#endif
    profile_print(m,dt);
    if(g_pilot_real) printf("PILOT_REAL: %ld cross-layer loads completed, %ld dropped (main already on the layer) | PILOT_K=%d\n",
...
/* SERVE mode (for the 'coli' CLI): loads the model ONCE, then a conversational CHAT.
 * PERSISTENT KV-cache across turns: history stays in cache, only NEW tokens get
 * prefilled -> the model REMEMBERS the conversation and doesn't reprocess the past
 * (lossless, more human, faster). GLM chat template with special tokens (CHAT_TEMPLATE=0 -> raw).
 * Protocol: "\x01\x01" "READY" "\x01\x01\n" after load; streamed reply; "\x01\x01" "END" "\x01\x01\n" at end of turn.
 * ":reset" (line "\x02RESET") clears memory. EOF -> exits. */
/* ---- RFC: LIVE RE-PIN (opt-in, REPIN=n, default OFF) ----
 * upstream AUTOPINs at START (from .coli_usage). This adds a between-turns re-pin: at
...
/* ---- KV ON DISK: the conversation reopens WARM (KVSAVE=0 disables it) ----
 * Re-prefilling a reopened chat costs hours on this disk; the compressed MLA KV
 * costs ~182 KB/token. Append-only file <SNAP>/.coli_kv: header (magic + dimensions +
 * nrec) and one record per position [tok i32][Lc+Rc of the 78 layers][DSA Ic]. At
 * the end of a turn ONLY the new positions get appended and nrec is rewritten last: a
 * crash mid-append leaves the old nrec = a consistent file. The MTP layer's KV row is
 * not saved: on resume kv_start=-1 and the draft window restarts on its own. */
...
static void kv_disk_append(Model *m, const int *hist, int len){
    ...
    fflush(f);                                   /* data first, counter after */
...
static void run_serve(Model *m, const char *snap){
    ...
    while((nr=getline(&line,&cap,stdin))>0){
        ...
        if(!strcmp(line,"\x02MORE")){                /* continues the reply truncated by NGEN:
            history is already in KV, just re-forward the LAST token to get the logits back */
            ...
            fflush(stdout); kv_disk_append(m,hist,len); repin_pass(m); continue; }   /* RFC: live re-pin between turns */
        ...
        int bl=0, k=0;                           /* builds/tokenizes the turn */
        /* OFFICIAL GLM-5.2 template (chat_template.jinja): no \n after the roles, and after
         * <|assistant|> the think block is ALWAYS needed — <think></think> DISABLES it (nothink):
         * with the wrong template the model rambles and never emits the stop. THINK=1 enables it. */
        ...
        if(raw_mode){
            ...
            if(prefix<old_len){
                ...
                kv_disk_truncate(m,len);           /* the next append only overwrites the tail */
            }
            k=prompt_tokens-len;
        ...
        else logit=step(m,hist+len-1,1,len-1);   /* identical prompt/prefix: regenerate the logits */
        EmitStream es={&T,m,now_s(),0,1};
        int prod=0;
        grammar_reset();                         /* new reply = new document (MORE continues instead) */
        if(cur>0) prod=spec_decode(m,hist,len,cur,eos,logit,emit_stream,&es,&len);
        else free(logit);
        double tdt=now_s()-tt0; if(tdt<1e-6) tdt=1e-6;
        double dh=(double)(m->hits-h0), dm=(double)(m->miss-ms0);
        ...
        kv_disk_append(m,hist,len);      /* KV on disk: the next startup resumes from here */
...
/* resident bytes of a tensor [O,I] at the given bit count (mirrors qt_bytes) */
...
/* TRUE bytes of an expert: from the container if pre-quantized, otherwise estimated from ebits */
 ...
 * "TIERS <vram> <ram> <disk> <vram_gb> <ram_gb>" on the protocol channel.
 * ram = non-VRAM pinned + current LRU; disk = everything else. */
/* BRAIN MAP (dashboard): per-turn expert hit bitmap + full residency/heat map.
 * g_ehit[layer][eid]=1 when the expert is routed this turn;
 * hits_emit serializes it and clears it. emap_emit snapshots tier+heat of ALL. */
...
/* HITS: 1bit/expert bitmap (same order as EMAP), then cleared for the next turn. */
static void hits_emit(Model *m){
...
/* dumps the expert usage histogram to a file: lines "layer eid count" (for PIN).
 * Includes the MTP row (layer n_layers). Atomic write (tmp+rename): it's also
 * called every serve turn and the process can die at any moment. */
...
/* LEARNING CACHE: PERSISTENT usage histogram in <SNAP>/.coli_usage.
 * Loaded at startup (counters resume from history), saved every turn:
 * the more you use colibri, the better auto-pin knows YOUR hot experts. */
...
/* HOT-STORE ("colibri's redis"): loads into RAM, ONCE and for good, the top experts
 * by measured usage frequency (STATS file from a previous run), within a GB budget.
 * Every hit avoids a read from the slow disk. */
/* MLOCK: wire pinned experts into physical RAM so macOS's memory compressor can't
...
static void pin_load(Model *m, const char *statspath, double gb){
    ...
    int *cnt_l=calloc(c->n_layers+1,sizeof(int));   /* +1: MTP row */
    ...
    fprintf(stderr,"[PIN] placement: %d VRAM + %d RAM experts (%.1f GB warm) in %.0fs from %s\n",
        m->gpu_expert_count,npin-m->gpu_expert_count,(npin-m->gpu_expert_count)*eb/1e9,now_s()-t0,statspath);
    pin_wire(m);                                   /* wire in RAM (no compression) */
...
/* RAM available RIGHT NOW (GB): it's the real ceiling, not the total. Linux: MemAvailable
 * from /proc/meminfo. macOS: free+inactive+purgeable pages from host_statistics64
 * (same semantics: reclaimable without swap). Without this branch the "assume 8 GB"
 * fallback was crippling the expert cache on exactly the machines with the most RAM. */
...
/* bytes available for experts (pin + LRU) within the budget — mirrors cap_for_ram's accounting */
...
/* clamps the expert cache to a RAM budget (GB): cap s.t. resident + cache + slack <= budget.
 * ram_gb<=0 -> AUTO budget = 88% of RAM available right now (leaves room for OS+wrapper:
 * overshooting = kernel OOM-kill mid-generation, much worse than a smaller cache). */
static void cap_for_ram(Model *m, double ram_gb, int ebits, int max_ctx){
    ...
    if(m->has_mtp) nsp+=2;                       /* MTP cache row: counts ~double (int8 expert = 2x int4) */
    int64_t eb=expert_bytes_probe(m,ebits);
    int auto_b = ram_gb<=0;
    if(auto_b){ ram_gb = g_mem_avail_boot*0.88;   /* measured BEFORE the load: the already-
                                                   * allocated resident set is subtracted below, not twice */
        if(ram_gb<4){ fprintf(stderr,"[RAM] MemAvailable is unreadable or too low; assuming 8 GB\n"); ram_gb=8; } }
    /* HONEST slack, not a flat guess (the 2026-07-04 OOM came from here):
     *  ws[64] working-set slab (ALL materialize in the prefill batch-union),
    ...
    /* PAGE-CACHE RESERVE (measured 2026-07-06): strangling it collapses buffered
     * preads from ~800 to ~180 MB/s — the last few GB of LRU return LESS than what
     * they cost in lost disk bandwidth. 2.5 GB always stays with the kernel. */
    double pc_b  = 2.5e9;
    ...
    if(capmax < m->ecap){
        ...
        /* AUTO-RAISE (issue #12): the budget allows MORE cache than requested.
         * Without this, a 128 GB machine ran with the LRU of a 16 GB one
         * (cap=8 by default in coli): 23-28% hit rate with dozens of GB unused.
         * Capped at n_experts: beyond that, every layer would have slots it can't fill.
         * CAP_RAISE=0 restores the fixed behavior. */
...
int main(int argc, char **argv){
    ...
    if(g_mmap) fprintf(stderr,"[MMAP] experts = zero-copy views into the files (page cache = cache)\n");
    ...
    if(strcmp(policy,"quality")&&strcmp(policy,"balanced")&&!experimental){
        fprintf(stderr,"COLI_POLICY invalid: quality, balanced or experimental-fast\n"); return 2;
    }
    ...
    g_mlock  = getenv("MLOCK")?atoi(getenv("MLOCK")):-1;   /* -1 auto (ON macOS), 0 off, 1 force */
    g_spec = getenv("SPEC")?atoi(getenv("SPEC")):1;
    g_draft = getenv("DRAFT")?atoi(getenv("DRAFT")):-1;
    g_no_fused_pair = getenv("COLI_NO_FUSED_PAIR")?atoi(getenv("COLI_NO_FUSED_PAIR")):0;   /* -1 = auto: 3 if MTP, 0 without */
    g_looka = getenv("LOOKA")?atoi(getenv("LOOKA")):0;    /* 1 = measures routing predictability */
    g_pilot = getenv("PILOT")?atoi(getenv("PILOT")):0;    /* 1 = router-driven prefetch */
    ...
    if(g_pilot_real) g_pilot=1;                           /* PILOT_REAL implies the pilot is active */
    ...
    g_idot = getenv("IDOT")?atoi(getenv("IDOT")):1;        /* 0 = exact f32 kernels (A/B) */
    ...
    g_absorb = getenv("ABSORB")?atoi(getenv("ABSORB")):-1; /* -1 auto: absorbed for S<=4 */
    g_dsa_force = getenv("DSA_FORCE")?atoi(getenv("DSA_FORCE")):0;
    /* matmul_qt documents the int4-IDOT threshold as "configurable via I4S" but the
     * getenv was missing, so the knob did nothing. I4S=<n> -> int4 IDOT only for S>=n. */
    if(getenv("I4S")) g_i4s=atoi(getenv("I4S"));
    g_temp = getenv("TEMP")?atof(getenv("TEMP")):-1;       /* -1 = auto (1.0 chat/text, greedy elsewhere) */
    ...
    if(g_draft>63) g_draft=63;                             /* -1 = auto, resolved after model_init */
    ...
    if(getenv("DSA_TOPK")) m.c.index_topk=atoi(getenv("DSA_TOPK"));   /* override for testing */
    ...
    /* also on stderr: it's the channel UIs (coli) show to the user */
     ...
     * Must come BEFORE cap_for_ram: pinned experts count toward the resident set. */
    if(getenv("PIN")){
     ...
     * at startup the most-used ones get auto-pinned in RAM (half the expert budget: the
     * pin knows YOUR history, the LRU adapts to the session). AUTOPIN=0 disables it. */
    { double ram_env = getenv("RAM_GB")?atof(getenv("RAM_GB")):0.0;
      ...
      if(!getenv("PIN") && autopin && hist>=5000){
          /* pin quota proportional to CONFIDENCE in the history: with little data the pin
           * picks the wrong experts and steals slots from the adaptive LRU; at steady
           * state (>=200k selections, a few hours of chat) it reaches half the expert budget. */
      ...
      /* ALWAYS: without a clamp the LRU grows to cap*76 layers = dozens of GB -> OOM-kill.
       * RAM_GB absent or <=0 = automatic budget from MemAvailable. */
      cap_for_ram(&m, ram_env, ebits, est_ctx); }
    const char *stats=getenv("STATS");   /* STATS=<file> -> expert usage histogram at end of run */

    /* benchmark scoring mode: SCORE=<requests.txt> -> log-likelihood per line */
    ...
    int n_new=nfull-np;
    /* The oracle (ref_glm.json in the repo) is from the TINY model: against the 744B it
     * gives 0/20 guaranteed on EVERY platform (tiny prompt tokens = garbage to the real
     * model). This is not an engine bug — see #76. */
    { int maxid=0; for(int i=0;i<nfull;i++) if(full[i]>maxid) maxid=full[i];
      if(m.c.vocab>1000 && maxid<1000 && !getenv("REF_FORCE")){
        fprintf(stderr,"ERROR: ref_glm.json is the TINY model's oracle (max token %d, but your vocab is %d).\n"
                       "       Engine self-test:  SNAP=./glm_tiny TF=1 ./glm 64 16 16   (expected 32/32)\n"
                       "       Real test:         PROMPT=\"Hello\" NGEN=32 SNAP=<model> ./glm 64\n"
