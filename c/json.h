/* Minimal JSON parser, header-only. Used for:
 *  - the safetensors file header (one big object name->{dtype,shape,data_offsets})
...
static char *j_parse_str_raw(jparser *p) {
    ...
    /* find the end handling escapes, then copy decoding the base cases */
