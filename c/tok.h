 ...
 *   - pre_tokenizer: Split regex (cl100k pattern) + ByteLevel(add_prefix_space=false)
...
static int u8_next(const unsigned char *s, int len, int i, uint32_t *cp){
    ...
    *cp=c; return 1;   /* invalid byte: treated as single */
}
...
static void tok_load(Tok *T, const char *path){
    ...
    /* vocab: string -> id  (capacity power of 2, ~2-3x) */
...
/* ---------- pre-tokenizer regex (cl100k pattern) on a portion of text ----------
 * Decodes the codepoints, applies the alternatives IN ORDER, and for each piece calls bpe_piece. */
    ...
    while(i<n){
        ...
        /* 5) \s*[\r\n]+  -> whitespace run up to the last contiguous newline */
        {
            ...
            if(r>i){ int last=-1; for(int j=i;j<r;j++) if(ISNL(cp[j])) last=j;
                ...
                /* 6) \s+(?!\S): if followed by non-space leave the last ws, otherwise take everything */
                int end = (r<n) ? r-1 : r;
...
/* ---------- decode: ids -> text (inverse byte-level; literal added tokens) ---------- */
