#!/usr/bin/env bash
# colibrì — installation on a fresh machine (Linux x86-64, macOS, Windows/MinGW).
...
# 2) build: native (fast, for THIS machine). For a distributable binary: make portable
...
Darwin)
    ...
    ;;
MINGW*|MSYS*)
    # MSYS2 provides /proc/meminfo as a symlink (more reliable than wmic, deprecated)
