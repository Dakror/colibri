#!/usr/bin/env bash
# GLM-5.2 pipeline (int4, streaming, 15 GB RAM) — all in WSL, model on ext4.
#   usage from c/:  scripts/run.sh ["prompt"] [n_tokens]
# Does: (1) waits for the move to ext4, (2) resumes the conversion until complete,
#     (3) builds the engine, (4) generates text staying within the RAM budget.
set -euo pipefail
...
# 2) resumes+completes the conversion (resumable: skips shards already done)
