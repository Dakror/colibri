...
/* feeds the walker a string; returns how many bytes were consumed */
...
int main(void){
    ...
    /* literal: the entire prefix is forced, then the parse terminates */
    ...
    /* alternate: forcing stops at the branch point and resumes after */
    ...
    /* #48-style enum: the quote is forced, then from the first byte the entire value */
    CHECK(gr_parse(&G,
        "root ::= \"\\\"\" val \"\\\"\"\n"
    ...
    /* classes with range, star: no forcing where the grammar branches */
    ...
    /* plus on group: forcing stops where the parse is terminable */
    ...
    /* postfix on multi-byte literal: repeats the ENTIRE literal */
    ...
    /* negated class: the union with the closure covers every byte -> no forcing */
    ...
    /* desync: a disallowed byte does NOT mutate the state */
    ...
    /* hex escapes and comments; rules spanning multiple lines */
    CHECK(gr_parse(&G,
        "# grammatica di prova\n"
        "root ::= \"\\x41\"   # una A\n"
        "         [\\x30-\\x32]\n")==0);
    ...
    /* parse errors: undefined rule, missing root, misplaced ')' */
    ...
    /* at end of line the parse is terminable (riga+): no forcing — the model can
