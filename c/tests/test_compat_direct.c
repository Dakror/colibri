 ...
 * compat_open_direct(path) must give an fd that bypasses the file system cache,
 * readable with pread() at offset/len/buffer aligned to 4K (same contract as
...
int main(void){
    ...
    /* unaligned request: must fail (-1), NEVER return wrong data */
     ...
     * returns -1); compat_fsize must work on both types of fd */
