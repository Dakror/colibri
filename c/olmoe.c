 ...
 * token ids as the reference (ref.json) -> validates the core before scaling to GLM-5.2.
 *
 * Dense (embed, attn, router, norms, lm_head) resident in RAM (float32).
 * Experts read from disk on-demand via pread+fadvise(DONTNEED), per-layer LRU cache.
 * Multi-thread matmul with OpenMP (no BLAS).
 */
#define _GNU_SOURCE
#include <stdio.h>
#include <stdlib.h>
 ...
 * NEON (sdot where dotprod is present) — same IDOT family as glm.c, IDOT=0 for
...
/* rmsnorm on a row of length D, in-place on out (out can be == x) */
...
/* ---------- RoPE on a single-head vector (head_dim) at absolute position pos ---------- */
...
/* attention on the new tokens x[S,hidden]; pos_base = absolute position of the first new token */
static void attention(Model *m, Layer *l, int layer, float *x, int S, int pos_base, float *out) {
    ...
    /* qk-norm on the whole hidden vector, then RoPE per head */
    for (int s = 0; s < S; s++) {
    ...
    int Tk = pos_base + S;             /* total number of available keys */
