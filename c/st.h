 ...
 *     peak RAM stays dense+cache, not the whole model. (see the mmap-rss-bug note)
...
typedef struct {
    ...
    int        n, cap;
    int        fds[512];
    int        dfds[512];  /* O_DIRECT twins (opened lazily): -2 = not yet tried */
    char      *paths[512];
    int        nfd;
    int       *hidx;      /* hash map name->index (open addressing): with ~120k tensors
                           * (GLM: 256 expert x 78 layer x 3 x 2) the linear scan
...
/* O_DIRECT twin fd of the same file (bypasses the page cache: the buffered read on
 * ext4-in-VHDX throttles to ~0.8 GB/s, O_DIRECT reaches 2.3+; measured). -1 if not available. */
static int st_direct_fd(shards *S, int fd) {
    for (int i = 0; i < S->nfd; i++) if (S->fds[i] == fd) return S->dfds[i];
    return -1;
}

/* indexes all model-*.safetensors in snap_dir */
static void st_init(shards *S, const char *snap_dir) {
    memset(S, 0, sizeof(*S));
    S->cap = 4096; S->t = calloc(S->cap, sizeof(st_tensor));
    /* collects the shard file names in order */
    ...
    for (int fi = 0; fi < nf; fi++) {
        ...
        free(arena); /* the jvals stay leaked: ok, one-time at startup */
        free(hdr);
    }
...
/* ASYNCHRONOUS prefetch: tells the kernel to start reading the tensor pages in
 * the background (readahead). Serves to overlap the experts' I/O with compute: the
 * whole expert set of a layer is prefetched, then the synchronous preads find the cache
...
/* reads a tensor into a caller-provided float32 buffer (numel floats).
 ...
 * Serves for GLM's fused experts (one tensor = block [E, ...]): only the
 * requested expert is read via pread of the sub-range, no reading of the whole block. */
