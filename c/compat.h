 ...
 * DONTNEED -> no-op: XNU does not expose a targeted per-range drop; its unified
 ...
 *             with a lot of RAM keeping the pages is exactly what you want). */
 ...
 * The equivalent is F_NOCACHE on the fd (bypass of the unified buffer cache).
 * compat_open_direct() opens the "twin" fd without cache, like the O_DIRECT twin
 * of st.h. The caller's 4K-aligned preads stay valid: F_NOCACHE does not
 * impose alignment constraints. */
 ...
 * hundred KB): first the working set is grown by len + margin, then it is locked.
 ...
 * NOTE: memory allocated with _aligned_malloc MUST be freed with
 ...
 * Opens the "twin" fd without file system cache, like the O_DIRECT twin of
 * st.h on Linux and F_NOCACHE on macOS. Same contract: the caller's offset, length and
 * buffer must be aligned to 4K (the expert slabs use
 * posix_memalign(4096) and the engine's DIRECT=1 path already aligns offset
 * and len); non-aligned requests fail with -1, never corrupt data.
 ...
 * UCRT): the size is requested directly from the kernel. Works on
...
/* --- setenv -> SetEnvironmentVariableA (POSIX setenv absent on Windows) --- */
 ...
 * On Linux/macOS, posix_memalign uses normal free(). */
