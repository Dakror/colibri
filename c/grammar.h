/* grammar.h — grammar draft (#48): GBNF (subset) evaluated at the BYTE level.
 *
 * Idea: in constrained-output workloads (JSON/NDJSON, function calling, structured
 * extraction) a fraction of the tokens is DETERMINISTIC given the grammar: brackets,
 * quotes, key names, separators, enum values. Those spans are free
 * drafts at acceptance ~1: no head, no lookup table — the
 * batch-union verification confirms them and pays ONE forward for several tokens. And it also hooks in where
 * the MTP int4 head does not start (#8).
 *
 * The grammar NEVER constrains sampling: it only proposes drafts, which the verification
 * accepts or rejects like any other draft. Wrong or out-of-sync grammar =>
 * rejected drafts, IDENTICAL output. It is a pure accelerator, never a filter.
 *
 * GBNF subset (llama.cpp style), evaluated on the BYTES:
 *   root ::= obj+                          # the start rule is called "root"
 *   obj  ::= "{" pair ("," pair)* "}" "\n"
 *   str  ::= "\"" [^"\\]* "\""
 * Supported: literals "..." (escapes \" \\ \n \r \t \xHH), classes [a-z0-9-] also
 * negated [^...], rule references, groups (...), postfixes ? * +, comments #,
 * alternates with |, epsilon as "". Rules can span multiple lines: a
 * new rule begins where an identifier is followed by "::=".
 * NOT supported: repetitions {m,n}, unicode ranges in classes (classes work
 * on bytes; for multibyte UTF-8 use the literals, which pass the raw bytes).
 ...
 * form has a terminal symbol (byte class) on top or is empty (parse
 * completable here). gr_forced() extends the prefix as long as there is A SINGLE legal
 * byte and the parse is not terminable: that prefix is the forced draft.
 */
...
#define GR_MAX_DEPTH  64      /* maximum depth of a PDA stack */

typedef struct { uint8_t bits[32]; } GrCls;              /* set of admitted bytes */
...
/* frame = position inside an alternate: (rule, alternate, symbol) */
...
/* postfix ? * + on the ITEM just read (symbols [n0, n) of the current alternate).
...
static int gr__postfix(Grammar *G, int ri, int ai, int n0, char op){
    ...
    if(gr__push(G,ri,ai,&R)) goto full;                   /* the item in the alternate becomes R */
    return 0;
full:
...
/* parse of the GBNF text. 0 = ok; -1 = error (message in G->err). */
...
/* brings the stack into normal form (top = terminal, or empty stack = complete parse),
 * branching on the alternates of the referenced rules. 0 = overflow (fail-safe). */
static int gr__normalize(Grammar *G, GrStack *k, GrState *out, int depth){
    for(;;){
        if(k->n==0) return gr__set_add(out,k);
        GrFrame *t=&k->f[k->n-1];
        GrAlt *A=&G->r[t->r].a[t->a];
        if(t->s>=A->n){ k->n--; continue; }               /* alternate exhausted: pop */
        GrSym *sy=&A->s[t->s];
...
/* advances by one byte. 1 = consumed; 0 = byte not admitted (state UNCHANGED);
 * -1 = walker off (overflow: from here on no more drafts). */
static int gr_accept(GrState *S, unsigned char b){
    if(!S->alive) return -1;
    GrState out; out.G=S->G; out.n=0; out.alive=1;
    for(int i=0;i<S->n;i++){
        GrStack *k=&S->st[i];
        if(k->n==0) continue;                             /* parse already complete: does not consume */
        GrFrame *t=&k->f[k->n-1];
...
/* set of bytes admitted now (256 bitmap). Returns the count;
 * *can_end = 1 if the parse can terminate here (so the model can emit EOS). */
...
/* FORCED prefix: extends as long as there is A SINGLE legal byte and the parse is not
